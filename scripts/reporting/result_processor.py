...
class ResultProcessor:
            ...
            comparison_data.append(
                {
                    ...
                    "success_rate": stats["problems_solved"] / stats["problems_applicable"]
                    if stats["problems_applicable"] > 0
                    else 0.0,
    ...
    def get_best_performers_by_library_and_type(
    ...
    def get_results_matrix(self, results: List[BenchmarkResult]) -> Dict[str, Any]:
        ...
        for result in results:
            if result.problem_name not in problem_metadata:
                ...
                if result.problem_name in problem_registry.get("problem_libraries", {}):
                    known_objective = problem_registry["problem_libraries"][result.problem_name].get(
                        "known_objective_value"
                    )
        ...
        return {"problems": problems_sorted, "solvers": solvers, "matrix": matrix, "problem_metadata": problem_metadata}
...
def main():
    ...
    if results:
        ...
        for solver_data in comparison:
            print(
                f"  {solver_data['solver_name']}: {solver_data['success_rate']:.2%} success, {solver_data['avg_solve_time']:.4f}s avg"
            )
