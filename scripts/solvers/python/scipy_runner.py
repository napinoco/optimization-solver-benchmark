...
class ScipySolver(SolverInterface):
    ...
    def _convert_sedumi_to_scipy_format(self, problem_data: ProblemData) -> Dict:
        ...
        if problem_data.A_ub is not None and problem_data.b_ub is not None:
            return {"A_ub": problem_data.A_ub, "b_ub": problem_data.b_ub, "bounds": problem_data.bounds}
        ...
        return {"A_ub": None, "b_ub": None, "bounds": [(0, None)] * n_vars}
    ...
    def _derive_scipy_constraints_from_cones(self, problem_data: ProblemData) -> Dict:
        ...
        return {"A_ub": None, "b_ub": None, "bounds": bounds}

    def _create_standardized_result(
        self,
        solve_time: float,
        status: str,
    ...
    def _solve_lp(self, problem_data: ProblemData, start_time: float) -> SolverResult:
        ...
        self.logger.debug(
            f"LP problem dimensions: c={c.shape if c is not None else None}, "
        ...
        # Solve using linprog
        result = linprog(
            c=c, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq, b_eq=b_eq, bounds=bounds, method=self.method, options=self.options
        )
        ...
        additional_info = {"solver_solve_time": solve_time}
        ...
        original_status = {
            ...
            "scipy_fun": float(result.fun) if result.fun is not None else None,
        }
    ...
    def _solve_qp(self, problem_data: ProblemData, start_time: float) -> SolverResult:
        ...
        if A_eq is not None and b_eq is not None:
            constraints.append({"type": "eq", "fun": lambda x: np.dot(A_eq, x) - b_eq, "jac": lambda x: A_eq})

        # Inequality constraints (A_ub * x <= b_ub)
        if A_ub is not None and b_ub is not None:
            constraints.append({"type": "ineq", "fun": lambda x: b_ub - np.dot(A_ub, x), "jac": lambda x: -A_ub})
        ...
        self.logger.debug(f"QP problem dimensions: Q={Q.shape}, c={c.shape}, constraints={len(constraints)}")
        ...
        self.logger.debug(
            f"QP solve completed: status={status}, objective={primal_objective_value}, time={solve_time:.3f}s"
        )
...
if __name__ == "__main__":
    ...
    try:
        invalid_problem = ProblemData(name="invalid", problem_class="INVALID", c=np.array([1.0]))
